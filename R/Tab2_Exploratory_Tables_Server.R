    ...
    moduleServer(id, function(input, output, session) {
        ...
        observe({
            ...
            updateSelectizeInput(
                session, "Tab2_Column1_Variable",
                choices = vchoices, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab2_Column2_Variable",
                choices = vchoices, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab2_Column3_Variable",
                choices = vchoices, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab2_Column4_Variable",
                choices = vchoices, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab2_Column5_Variable",
                choices = vchoices, selected = "None Selected", server = TRUE
            )
        })
        ...
        Create_Table <- metaReactive2({
            if (
                input$Tab2_Column1_Variable == "None Selected" &
                ...
                return(validate("Please upload data and select data columns in sequential order."))
            } else if (
                input$Tab2_Column1_Variable != "None Selected" &
                input$Tab2_Column2_Variable == "None Selected" &
                input$Tab2_Column3_Variable == "None Selected" &
                ...
                metaExpr({ ..(datalist[[data]]())[,c(..(input$Tab2_Column1_Variable),
        ...
        output$Table <- metaRender(renderDataTable, {
            DT::datatable(
                ...
                options = list(
                    ...
                    pageLength = 30, scrollX = TRUE, scrollY = length_px
                )
            )
        })
