    ...
    moduleServer(id, function(input, output, session) {
        ...
        observe({
            ...
            updateSelectizeInput(
                session, "Tab6_Select_Categorical_Variable_1",
                choices = vchoicesASS, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab6_Select_Categorical_Variable_2",
                choices = vchoicesASS, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab6_Select_Continuous_Variable_1",
                choices = vchoicesASS, selected = "None Selected", server = TRUE
            )
        })
        ...
        Chi <- metaReactive2({
            validate(
                need(
                    !is.null(rowselect()) | !is.null(datalist[["patient_manual_data"]]()) |
                        !is.null(datalist[["sample_manual_data"]]()) | !is.null(datalist[["CNA_manual_data"]]()),
            ...
            if (
                ...
                metaExpr({
                    for (i in 1:length(c(..(input$Tab6_Select_Categorical_Variable_2)))) {
                        cat(
                            noquote(
                                paste(
                        ...
                        print(stats::chisq.test(..(data_Association2())[[i]]))
                    }
                })
            }
        })
            ...
            if (
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Categorical_Variable_2))) {
                        ...
                        Chi_List[[name]] <- stats::chisq.test(..(data_Association2())[[i]])
                    }
                    Chi_List
                })
            }
        ...
        data_Association1Ad <- metaReactive2({
            validate(
                need(
                    !is.null(rowselect()) | !is.null(datalist[["patient_manual_data"]]()) |
                        !is.null(datalist[["sample_manual_data"]]()) | !is.null(datalist[["CNA_manual_data"]]()),
                    paste("Please select cBioPortal dataset or upload your own clinical file, sample file and/or CNA file.")
                )
            )
            validate(
                need(
                    ...
                    paste("Please only select cBioPortal dataset OR upload your own data.")
                )
            )
            validate(
                need(
            ...
            if (
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Categorical_Variable_2))) {
                        ...
                        Test <- stats::chisq.test(..(data_Association2())[[i]])
                    ...
                    Table$Adj_Pval <- signif(
                        p.adjust(
                            Table$Pval, method = "BH",
                            n = length(..(input$Tab6_Select_Categorical_Variable_2))
                        ),
                        digits = 3
                    )
        ...
        output$Cat1Ad <- metaRender(renderDataTable, {
            datatable(
                ..(data_Association1Ad()),
                options = list(
                    lengthMenu = c(10, 30, 50, 100), pageLength = 10, scrollX = TRUE
                )
            )
        })
    })
...
Tab6_FE_Sim_Server <- function(id, datalist, datalist1, data, rowselect) {
    moduleServer(id, function(input, output, session) {
        ...
        observe({
            ...
            updateSelectizeInput(
                session, "Tab6_Select_Categorical_Variable_3",
                choices = vchoicesASS, selected = "None Selected", server = TRUE
            )
            updateSelectizeInput(
                session, "Tab6_Select_Continuous_Variable_1",
...
Tab6_FE_Server <- function(id, datalist, datalist1, data, rowselect) {
    moduleServer(id, function(input, output, session) {

        observe({
            vchoicesASS <- c(names(datalist1[[data]]()), "None Selected")
            updateSelectizeInput(session, "Tab6_Select_Categorical_Variable_1",
                                 choices = vchoicesASS, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab6_Select_Categorical_Variable_2",
                                 choices = vchoicesASS, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab6_Select_Categorical_Variable_3",
                                 choices = vchoicesASS, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab6_Select_Continuous_Variable_1",
                                 choices = vchoicesASS, selected = "None Selected", server = TRUE)
        })
        ...
        data_Association2 <- metaReactive2({
            metaExpr({
                my_list <- list()
                for(i in 1:length(..(input$Tab6_Select_Categorical_Variable_2))) {
                    my_list[[i]] <- table(..(datalist1[[data]]())[,..(input$Tab6_Select_Categorical_Variable_1)],
                                          ..(datalist1[[data]]())[, ..(input$Tab6_Select_Categorical_Variable_2)[i]])
                }
                my_list
            })
        ...
        Fis <- metaReactive2({
            validate(need(
                ...
                paste("\n \n \n \n \n \n Please select cBioPortal dataset or upload your own clinical file, sample file and/or CNA file. \n \n \n",
                      "\n", "\n", "\n", "\n")))
            validate(need(
                c(is.null(datalist[["patient_manual_data"]]()) &
                      is.null(datalist[["sample_manual_data"]]()) &
            ...
            validate(need(!is.null(input$Tab6_Select_Categorical_Variable_2),
                          "Please select categorical variables of interest."))
            if (isTRUE(input$Tab6_Select_Categorical_Variable_1 == "None Selected" |
                       input$Tab6_Select_Categorical_Variable_2 == "None Selected")) {
                return(NULL)
            } else {
                metaExpr({
                    for (i in 1:length(c(..(input$Tab6_Select_Categorical_Variable_2)))) {
                        tryCatch({
                            cat(noquote(paste("Categorical Variable 1:",
...
Tab6_ANOVA_Server <- function(id, datalist, datalist1, data, rowselect) {
    moduleServer(id, function(input, output, session) {

        observe({
            vchoicesASS <- c(names(datalist1[[data]]()), "None Selected")
            updateSelectizeInput(session, "Tab6_Select_Categorical_Variable_1",
                                 choices = vchoicesASS, selected = "None Selected",
                                 server = TRUE)
            updateSelectizeInput(session, "Tab6_Select_Categorical_Variable_2",
                                 choices = vchoicesASS, selected = "None Selected",
                                 server = TRUE)
        ...
        Assump2 <- metaReactive2({
            validate(need(!is.null(rowselect()) | !is.null(datalist[["patient_manual_data"]]()) |
                              !is.null(datalist[["sample_manual_data"]]()) |
                              !is.null(datalist[["CNA_manual_data"]]()),
                ...
                metaExpr({
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        cat(noquote(paste("Categorical Variable:",
                                          ...
                                          ..(input$Tab6_Select_Continuous_Variable_1)[i],
                                          "\n")))
                        print(stats::fligner.test(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
                                ..(datalist1[[data]]())[
                                    ,..(input$Tab6_Select_Categorical_Variable_3)],
                            ..(datalist1[[data]]())))
                ...
                metaExpr({
                    for (i in 1:length(c(..(input$Tab6_Select_Continuous_Variable_1)))) {
                        cat(noquote(paste("Categorical Variable:",
                                          ...
                                          ..(input$Tab6_Select_Continuous_Variable_1)[i],
                                          "\n", "\n")))
                        Data1 <- ..(datalist1[[data]]()) %>% group_by(!!as.name(..(input$Tab6_Select_Categorical_Variable_3))) %>%
                            rstatix::shapiro_test(!!as.name(..(input$Tab6_Select_Continuous_Variable_1)[i]))
        ...
        Shapiro_Code <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        name <- ..(input$Tab6_Select_Continuous_Variable_1)[i]
                        Shapiro_List[[name]] <- stats::shapiro.test(
                            ..(datalist1[[data]]())[
                                ,..(input$Tab6_Select_Continuous_Variable_1)[i]])
                        Shapiro_List
                    }
        ...
        ANOVA_R <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    for (i in 1:length(c(..(input$Tab6_Select_Continuous_Variable_1)))) {
                        res.aov <- stats::aov(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
                                ..(datalist1[[data]]())[
                                    ,..(input$Tab6_Select_Categorical_Variable_3)],
                            data = ..(datalist1[[data]]()))
        ...
        ANOVA_Code <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        name <- ..(input$Tab6_Select_Continuous_Variable_1)[i]
                        ANOVA_List[[name]] <-  summary(stats::aov(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
                                ..(datalist1[[data]]())[
                                    ,..(input$Tab6_Select_Categorical_Variable_3)],
                            data = ..(datalist1[[data]]())))
        ...
        data_ANOVAAd <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    ...
                    for (i in 1:length(c(..(input$Tab6_Select_Continuous_Variable_1)))) {
                        Variables <- c(Variables, paste(..(input$Tab6_Select_Categorical_Variable_3),
                                                        "&", ..(input$Tab6_Select_Continuous_Variable_1)[i], sep=" "))
                        Test <- summary(stats::aov(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
                                ..(datalist1[[data]]())[
                                    ,..(input$Tab6_Select_Categorical_Variable_3)],
                            data = ..(datalist1[[data]]())))
                    ...
                    Table <- cbind.data.frame(Variables, Pval)
                    Table$Adj_Pval <- signif(p.adjust(Table$Pval, method = "BH",
                                                      n=length(..(input$Tab6_Select_Continuous_Variable_1))), digits=3)
                    Table %>% select(Variables, Pval, Adj_Pval)
                })
            }
        })
        ...
        output$ANOVAAd <- metaRender(renderDataTable, {
            DT::datatable(..(data_ANOVAAd()), options = list(lengthMenu = c(10, 30, 50, 100),
...
Tab6_KW_Server <- function(id, datalist, datalist1, data, rowselect) {
    moduleServer(id, function(input, output, session) {
        ...
        KW_R <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        Kw <- stats::kruskal.test(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
        ...
        KW_Code <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        name <- ..(input$Tab6_Select_Continuous_Variable_1)[i]
                        KW_List[[name]] <- stats::kruskal.test(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
                                ..(datalist1[[data]]())[,..(input$Tab6_Select_Categorical_Variable_3)],
                            data = ..(datalist1[[data]]()))
                    }
        ...
        data_KWAdj <- metaReactive2({
            validate(need(!is.null(rowselect()) | !is.null(datalist[["patient_manual_data"]]()) |
                              !is.null(datalist[["sample_manual_data"]]()) |
                              !is.null(datalist[["CNA_manual_data"]]()),
                          paste("Please select cBioPortal dataset or upload your own clinical file,",
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        Variables <- c(Variables, paste(..(input$Tab6_Select_Categorical_Variable_3),
                                                        "&", ..(input$Tab6_Select_Continuous_Variable_1)[i], sep=" "))
                        Test <- stats::kruskal.test(..(datalist1[[data]]())[
                            ,..(input$Tab6_Select_Continuous_Variable_1)[i]] ~
                                ..(datalist1[[data]]())[,..(input$Tab6_Select_Categorical_Variable_3)],
                            data = ..(datalist1[[data]]()))
...
Tab6_PWC_Server <- function(id, datalist, datalist1, data, rowselect) {
    moduleServer(id, function(input, output, session) {
        ...
        PWC <- metaReactive2({
            ...
            validate(need(!is.null(input$Tab6_Select_Continuous_Variable_1),
                          paste("\n \n \n Please select continuous variables of interest.",
                                "\n", "\n", "\n", "\n")))
            validate(need(input$Tab6_Select_Continuous_Variable_1 != "None Selected" &
                              input$Tab6_Select_Categorical_Variable_3 != "None Selected",
        ...
        WT_Code <- metaReactive2({
            ...
            if (isTRUE(input$Tab6_Select_Continuous_Variable_1 == "None Selected" |
                ...
                metaExpr({
                    ...
                    for (i in 1:length(..(input$Tab6_Select_Continuous_Variable_1))) {
                        name <- ..(input$Tab6_Select_Continuous_Variable_1)[i]
                        PairwiseT_List[[name]] <- stats::pairwise.t.test(
                            ..(datalist1[[data]]())[,..(input$Tab6_Select_Continuous_Variable_1)[i]],
                            ..(datalist1[[data]]())[,..(input$Tab6_Select_Categorical_Variable_3)],
                            p.adjust.method = "BH")
                    }
...
Tab6_CG_Server <- function(id, datalist, datalist1, data, rowselect) {
    moduleServer(id, function(input, output, session) {

        observe({
            vchoicesCG <- c(names(datalist1[[data]]()), "None Selected")
            updateSelectizeInput(session, "Tab6_Select_Response_Variable",
                                 choices = vchoicesCG, selected = "None Selected",
                                 server = TRUE)
            updateSelectizeInput(session, "Tab6_Select_Explanatory_Variable",
                                 choices = vchoicesCG, selected = "None Selected",
                                 server = TRUE)
        })
        ...
        Compare_Code <- metaReactive2({
            ...
            validate(need(c(is.null(datalist[["patient_manual_data"]]()) &
                          ...
                          paste("\n \n \n Please only select cBioPortal",
                                ...
                                "\n", "\n", "\n", "\n")))
            validate(need(!is.null(input$Tab6_Select_Response_Variable) &&
                              !is.null(input$Tab6_Select_Explanatory_Variable),
                          paste("\n \n \n Please select response and explanatory",
                                "variables. \n \n \n \n")))
            validate(need(input$Tab6_Select_Response_Variable != "None Selected" &
                              input$Tab6_Select_Explanatory_Variable != "None Selected",
                          paste("\n \n \n Please select response and explanatory",
                                "variables. \n \n \n \n")))
            if (isTRUE(input$Tab6_Select_Response_Variable == "None Selected" |
                       input$Tab6_Select_Explanatory_Variable == "None Selected")) {
                return(NULL)
            } else {
                metaExpr({
                    res <- compareGroups::compareGroups(
                        as.formula(paste(..(input$Tab6_Select_Response_Variable),
                                         '~', paste(c(..(input$Tab6_Select_Explanatory_Variable)),
                                                    collapse = "+"))),
                        data = ..(datalist1[[data]]()), max.ylev = 10, method = NA)
                    res
                })
            }
        })
