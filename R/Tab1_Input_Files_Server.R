            ...
            metaExpr({
                tryCatch({
                    cbio <- cBioPortalData::cBioPortal(
                        ...
                        protocol = "https", api. = "/api/api-docs"
                    )
       ...
       # Clinical data
        ...
        API_data_output <-  metaReactive2({
            if (is.null(rowselect())) {
                ...
                validate(
                    need(!is.null(API_Out()), "Error: Download failed, please select another dataset to analyse."))
                return(NULL)
            } else {
                tryCatch({
                    metaExpr({
        ...
        dataClinical <- metaReactive2({
            if (is.null(input$Input_Sample_File) & !is.null(input$Input_Patient_File) & is.null(API_data_output())) {
                ...
                validate(
                    need(ncol(sample_data()) > 2,
                         "Inputted file only has one column, please select file delimiters and options carefully.")
                )
                validate(
                    need("PATIENT_ID" %in% colnames(sample_data()),
                         "Inputted file does not have PATIENT_ID information - Are you using the right file?")
                )
                metaExpr({ ..(sample_data()) })
            } else if (is.null(input$Input_Sample_File) & is.null(input$Input_Patient_File) & is.null(API_data_output())) {
                validate(
                    need(!is.null(API_data_output()) | !is.null(input$Input_Sample_File) | !is.null(input$Input_Patient_File),
                ...
                validate(need(!is.null(API_data_output()[["patient"]]) | !is.null(API_data_output()[["sample"]]),
                              "Selected cBioPortal dataset does not contain clinical data, please select another dataset"))
                metaExpr({
                    if (!is.null(API_data_output()[["patient"]]) & is.null(API_data_output()[["sample"]])) {
                    validate(need(ncol(API_data_output()[["patient"]]) > 2,
                                  "Inputted file only has one column, please select file delimiters and options carefully."))
                    validate(need("PATIENT_ID" %in% colnames(API_data_output()[["patient"]]),
                                  "Inputted file does not have PATIENT_ID information - Are you using the right file?"))
                    API_data_output()[["patient"]]
                } else if (is.null(API_data_output()[["patient"]]) & !is.null(API_data_output()[["sample"]])) {
            validate(need(ncol(API_data_output()[["sample"]]) > 2,
                          "Inputted file only has one column, please select file delimiters and options carefully."))
            validate(need("PATIENT_ID" %in% colnames(API_data_output()[["sample"]]),
                          "Inputted file does not have PATIENT_ID information - Are you using the right file?"))
            ...
            merge(..(API_data_output()[["patient"]]), ..(API_data_output()[["sample"]]), by.x = "PATIENT_ID", by.y = "PATIENT_ID")
                ...
                validate(need(is.null(API_data_output()) | c(is.null(input$Input_Sample_File) & is.null(input$Input_Patient_File)),
        ...
        CNA_Validated <- metaReactive2({
            if (is.null(input$Input_CNA_File) & is.null(API_data_output())) {
                validate(need(!is.null(input$Input_CNA_File) |
                ...
                validate(need(ncol(CNA_data()) > 1,
                              "Inputted file only has one column, please select file delimiters and options carefully."))
                validate(need("Hugo_Symbol" %in% colnames(CNA_data()),
                              "Inputted file does not have Gene information (Hugo_Symbol) - Are you using the right file?"))
                metaExpr({ ..(CNA_data()) })
            } else if (is.null(input$Input_CNA_File) & !is.null(API_data_output())) {
                validate(need(!is.null(API_data_output()[["CNA"]]),
                              "Selected cBioPortal dataset does not contain the desired CNA data, please select another dataset"))
                validate(need(ncol(API_data_output()[["CNA"]]) > 1,
                              "Inputted file only has one column, please select file delimiters and options carefully."))
                validate(need("Hugo_Symbol" %in% colnames(API_data_output()[["CNA"]]),
                              "Inputted file does not have Gene information (Hugo_Symbol) - Are you using the right file?"))
                metaExpr({
                    ..(API_data_output()[["CNA"]])
                })
            } else {
                validate(need(is.null(API_data_output()) | is.null(input$Input_CNA_File),
                              "Please only select cBioPortal dataset OR upload your own data."))
            }
        })
        ...
        MAF_Validated <- metaReactive2({
            if (is.null(input$Input_MAF_File) & is.null(API_data_output())) {
                validate(need(!is.null(input$Input_MAF_File) |
                                  !is.null(API_data_output()),
                ...
                validate(need(ncol(MAF_data()) > 1,
                              "Inputted file only has one column, please select file delimiters and options carefully."))
                validate(need("Hugo_Symbol" %in% colnames(MAF_data()),
                              "Inputted file does not have Gene information (Hugo_Symbol) - Are you using the right file?"))
                metaExpr({ ..(MAF_data()) })
            } else if (is.null(input$Input_MAF_File) & !is.null(API_data_output())) {
                validate(need(!is.null(API_data_output()[["MAF"]]),
                              "Selected cBioPortal dataset does not contain mutation data, please select another dataset"))
                validate(need(ncol(API_data_output()[["MAF"]]) > 1,
                              "Inputted file only has one column, please select file delimiters and options carefully."))
                validate(need("Hugo_Symbol" %in% colnames(API_data_output()[["MAF"]]),
                              "Inputted file does not have Gene information (Hugo_Symbol) - Are you using the right file?"))
                metaExpr({ ..(API_data_output()[["MAF"]]) })
            } else {
                validate(need(is.null(API_data_output()) | is.null(input$Input_MAF_File),
                              "Please only select cBioPortal dataset OR upload your mutation own data."))
            }
        })


        rlist <- list(patient_manual_data = patient_data,
...
Tab1_Input_Files_Preview_Server <- function(id, datalist, data, length_px, select_dt) {
    moduleServer(id, function(input, output, session) {
        ...
        output$Preview <- metaRender(renderDataTable, {
            metaExpr({
                datatable(..(datalist[[data]]()), selection = select_dt,
                          options = list(
                              lengthMenu = c(10, 30, 50, 100),
                              pageLength = 30,
                              scrollX = TRUE,
