...
Tab3_Factor_Levels_UI <- function(id) {
    ...
    fluidRow(
        box(
            ...
            sidebar = boxSidebar(
                id= "Tab3_Convert_Type_Sidebar", width = 25,
                background = "#599740", icon = icon("rectangle-list"),
                h4(strong("Convert to Numeric")),
                selectInput(
                    inputId = ns("Tab3_Variables_to_Numeric"),
                    label = "Select Variables:", choices = "",
                    multiple = TRUE, width = "97%"
                ),
        ...
        box(
            ...
            shinycssloaders::withSpinner(
                verbatimTextOutput(ns("DataLevels")),
                proxy.height = "280px"
            )
        )
    )
...
Tab3_Subset_UI <- function(id) {
    ...
    fluidRow(
        box(
            ...
            sidebar = boxSidebar(
                id = "Tab3_Subset_Sidebar", width = 25,
                background = "#599740", icon = icon("rectangle-list"),
                h4(strong("Options")),
                selectInput(
                    ns("Tab3_Subset_Variable_1"),
                    "1) Filter Based on Variable:", choices = "PATIENT_ID"
                ),
                selectInput(
                    ns("Tab3_Subset_Variable_Levels_1"),
                    label = "Choose Variable Level:",
                    choices = "None Selected", selected = "None Selected",
                    multiple = TRUE
                ),
                selectInput(
                    ns("Tab3_Subset_Variable_2"),
                    "2) Filter Based on Variable:", choices = "PATIENT_ID"
                ),
                selectInput(
                ...
                selectInput(
                    ...
                    "3) Filter Based on Variable:", choices = "PATIENT_ID"
                ),
                selectInput(
        ...
        box(
            ...
            shinycssloaders::withSpinner(
                verbatimTextOutput(ns("TableLevels")),
                proxy.height = "180px"
            )
        )
    )
...
Tab3_Recode_UI <- function(id) {
    ...
    fluidRow(
        box(
            ...
            tags$hr(),
            selectInput(
                ns("Tab3_Select_OS_Column"),
                "Select OS Column:", choices = ""
            ),
            selectInput(
                ns("Tab3_Select_DSS_Column"),
                "Select DSS Column:", choices = ""
            ),
            tags$hr(),
        ...
        box(
            ...
            shinycssloaders::withSpinner(
                dataTableOutput(ns("TableRecode")), proxy.height = "480px"
            )
        )
    )
}
            ...
            sidebar = boxSidebar(
                ...
                conditionalPanel(
                    ...
                    prettyToggle(
                        ...
                        label_on = "Remove NAs", label_off = "Remove NAs",
                    ...
                    prettyToggle(
                        ...
                        label_on = "Segment Data", label_off = "Segment Data",
                        icon_on = fontawesome::fa_i(name = "check", verify_fa = FALSE),
                        icon_off = fontawesome::fa_i(name = "times", verify_fa = FALSE)
                    ),
                    numericInput(
                        inputId = ns("Tab3_Number_of_Segments"),
                        label = "Number of Segments:", value = 4, width = "95%"
                    )
                )
            ),
...
Tab3_Data_Down_UI <- function(id) {
    ...
    fluidRow(
        box(
            style = "height:560px",
            sidebar = boxSidebar(
                id = "Tab3_CNA_Download_Sidebar", width = 25,
                background = "#599740", icon = icon("rectangle-list"),
                h4(strong("Options")),
                awesomeRadio(
            ...
            shinycssloaders::withSpinner(
                dataTableOutput(ns("TableData")), proxy.height = "510px"
            )
        )
    )
}
