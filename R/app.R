    ...
    ui <- dashboardPage(
        ...
        body = dashboardBody(
            ...
            tabItems(tabItem(tabName = "input_files_1",
                     ...
                     tabItem(tabName = "AdjSurvival", Tab8_Adjusted_Survival_Curves_UI("tab8_Adjusted_Survival")),
                     ...
                     tabItem(tabName = "MAFText", Tab10_MAF_Text_Summary_UI("tab10_mafsummary_text")),
                     tabItem(tabName = "MAFVis", Tab10_MAF_Visual_Summary_UI("tab10_mafsummary_plot")),
    ...
    server <- function(input, output, session) {
        ...
        selectedLine <- reactive({
            if (is.null(input$`tab1_input_API-cBioData_rows_selected`)) {
                ...
                rows_selected <- as.numeric(input$`tab1_input_API-cBioData_rows_selected`)
            }
        })

        datalist <- Tab1_Input_Files_Manual_Server("tab1_input_manually", rowselect = selectedLine)
        ...
        Tab3_Render_Print_Server("tab3_factor_levels", Formatted_Data, "formatted_data",
        ...
        Tab3_Download_Server("tab3_data_download", Final_Data, "Final_DF")
         ...
         ## Treatment KM Curves
         Plot_KM_Treat_Yes <- Tab5_KM_Plot_Treatment_Server("tab5_km_treat_plot",  Final_Data, "Final_DF")
        ...
        Tab7_Uni_Cox_Server("tab7_Cox_Uni", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
        Tab7_Multi_Cox_Server("tab7_Cox_Multi", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
        Assumption_Plot <- Tab7_Multi_Cox_Server("tab7_Cox_Multi", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
        Download_Server("tab7_Cox_Multi", Assumption_Plot, "A_Plot")

        # Tab 8) Adjusted Survival Curves
        Tab8_Adjusted_Survival_Curves_Server("tab8_Adjusted_Survival", datalist,  Final_Data, "Final_DF",
                                             rowselect = selectedLine, previous_model = Assumption_Plot[["Previous_Model"]])
        PlotsAdjSurv <- Tab8_Adjusted_Survival_Curves_Server("tab8_Adjusted_Survival", datalist,  Final_Data, "Final_DF",
                                                             rowselect = selectedLine, previous_model = Assumption_Plot[["Previous_Model"]])
        Download_Server("tab8_Adjusted_Survival", PlotsAdjSurv, "Adjusted_Survival_Curves")

        # Tab 9)
        ## Rpart
        Tab9_RPST_Rpart_Server("tab9_Rpart_Tree", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
        Plot_Rpart <- Tab9_RPST_Rpart_Server("tab9_Rpart_Tree", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
        Download_Server_1("tab9_Rpart_Tree", Plot_Rpart, "Rpart_plot")
        Download_Server_2("tab9_Rpart_Tree", Plot_Rpart, "Rpart_KM_Curves")

        ## Ctree
        Tab9_RPST_Ctree_Server("tab9_Ctree_Tree", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
        Plot_Ctree <- Tab9_RPST_Ctree_Server("tab_Ctree_Tree", datalist,  Final_Data, "Final_DF", rowselect = selectedLine)
