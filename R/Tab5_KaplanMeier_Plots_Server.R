...
Tab5_KM_Plot_Server <- function(id, datalist, data) {
    moduleServer(id, function(input, output, session) {
        ...
        observe({
            ...
            updateSelectizeInput(session, "Tab5_KM_Clinical_Survival_Time",
                                 choices = vchoicesSurv1, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_Clinical_Event_Status",
                                 choices = vchoicesSurv1, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_Clinical_Select_Variable",
                                 choices = vchoicesSurv1, selected = "None Selected", server = TRUE)
        })
        ...
        datafit <- metaReactive2({
            metaExpr({
                survival::survfit(
                    Surv(as.numeric(Time), as.numeric(as.character(Cen))) ~ Strata,
                    data = ..(surv_data())
                )
            })
        })
        ...
        KMC1 <- metaReactive2({
            if (is.null(datalist[[data]]()) | input$Tab5_KM_Clinical_Survival_Time == "None Selected" |
                ...
                metaExpr({
                    survminer::ggsurvplot(
                        ..(datafit()), censor.shape = "", data = ..(surv_data()), size = 1,
                        conf.int = ..(input$Tab5_KM_Clinical_Display_CI),
                        pval = ..(input$Tab5_KM_Clinical_Display_Pval),
                        risk.table = ..(input$Tab5_KM_Clinical_Display_Risk_Table),
                        legend = c(..(input$Tab5_KM_Clinical_Legend_Position)),
                        ...
                        legend.labs = rownames(summary(..(datafit())$table)),
                        pval.size = 6, risk.table.height = 0.25,
                        ggtheme = theme_bw() +
        ...
        Logrank1 <- metaReactive2({
            if (is.null(datalist[[data]]()) | input$Tab5_KM_Clinical_Survival_Time == "None Selected" |
                ...
                metaExpr({
                    survival::survdiff(
                        Surv(..(datalist[[data]]())[,..(input$Tab5_KM_Clinical_Survival_Time)],
                             as.numeric(..(datalist[[data]]())[,..(input$Tab5_KM_Clinical_Event_Status)])) ~
...
Tab5_KM_Plot_Quart_Server <- function(id, datalist, data) {
    moduleServer(id, function(input, output, session) {
        ...
        observe({
            ...
            updateSelectizeInput(session, "Tab5_KM_CNA_Survival_Time",
                                 choices = vchoicesSurv2, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_CNA_Event_Status",
                                 choices = vchoicesSurv2, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_CNA_Select_Variable",
                                 choices = vchoicesSurv2, selected = "None Selected", server = TRUE)
        })
        ...
        PercentSurvPlot <- metaReactive2({
            if (is.null(datalist[[data]]()) | input$Tab5_KM_CNA_Survival_Time == "None Selected" |
        ...
        Logrank2 <- metaReactive2({
            if (is.null(datalist[[data]]()) | input$Tab5_KM_CNA_Survival_Time == "None Selected" |
                ...
                metaExpr({
                    survival::survdiff(
                        Surv(..(datalist[[data]]())[,..(input$Tab5_KM_CNA_Survival_Time)],
                             as.numeric(..(datalist[[data]]())[,..(input$Tab5_KM_CNA_Event_Status)])) ~
...
Tab5_KM_Plot_Treatment_Server <- function(id, datalist, data) {
    moduleServer(id, function(input, output, session) {
        ...
        observe({
            ...
            updateSelectizeInput(session, "Tab5_KM_Treatment_Survival_Time",
                                 choices = vchoicesSurvsplit, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_Treatment_Event_Status",
                                 choices = vchoicesSurvsplit, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_Treatment_Select_Variable",
                                 choices = vchoicesSurvsplit, selected = "None Selected", server = TRUE)
            updateSelectizeInput(session, "Tab5_KM_Treatment_Variable",
                                 choices = vchoicesSurvsplit, selected = "None Selected", server = TRUE)
        })
        ...
        KMR1Plot <- metaReactive2({
            if (is.null(datalist[[data]]()) |
                ...
                metaExpr({
                    survminer::ggsurvplot(
                        ..(datafit_R()), censor.shape = "",
                        xlab = ..(input$Tab5_KM_Treatment_Yes_X_Axis_Title),
                        ylab = ..(input$Tab5_KM_Treatment_Yes_Y_Axis_Title),
                        data = ..(surv_data_R()), size = 1,
                        conf.int = ..(input$Tab5_KM_Treatment_Yes_Display_CI),
                        pval = ..(input$Tab5_KM_Treatment_Yes_Display_Pval),
                        risk.table = ..(input$Tab5_KM_Treatment_Yes_Display_Risk_Table),
                        legend.title = ..(input$Tab5_KM_Treatment_Yes_Legend_Title),
                        legend = c(..(input$Tab5_KM_Treatment_Yes_Legend_Position)),
                        legend.labs = rownames(summary(..(datafit_R())$table)),
                        pval.size = 6, risk.table.height = 0.25,
                        ggtheme = theme_bw() +
                            theme(plot.title = element_text(size = 18, hjust = 0.5)) +
                            theme(legend.title = element_text(colour = "black", size = 15, face = "bold")),
        ...
        Logrank3 <- metaReactive2({
            if (is.null(datalist[[data]]()) |
                ...
                metaExpr({
                    survival::survdiff(
                        Surv(..(dataRYes())[,..(input$Tab5_KM_Treatment_Survival_Time)],
                             as.numeric(..(dataRYes())[,..(input$Tab5_KM_Treatment_Event_Status)])) ~
        ...
        Logrank4 <- metaReactive2({
            if (is.null(datalist[[data]]()) |
                ...
                metaExpr({
                    survival::survdiff(
                        Surv(..(dataRNo())[,..(input$Tab5_KM_Treatment_Survival_Time)],
                             as.numeric(..(dataRNo())[,..(input$Tab5_KM_Treatment_Event_Status)])) ~
        ...
        KMR2Plot <- metaReactive2({
            if (is.null(datalist[[data]]()) |
                ...
                metaExpr({
                    survminer::ggsurvplot(
                        ..(datafit_NR()), censor.shape = "",
                        xlab = ..(input$Tab5_KM_Treatment_No_X_Axis_Title),
                        ylab = ..(input$Tab5_KM_Treatment_No_Y_Axis_Title),
                        data = ..(surv_data_NR()), size = 1,
                        conf.int = ..(input$Tab5_KM_Treatment_No_Display_CI),
                        pval = ..(input$Tab5_KM_Treatment_No_Display_Pval),
                        risk.table = ..(input$Tab5_KM_Treatment_No_Display_Risk_Table),
                        legend.title = ..(input$Tab5_KM_Treatment_No_Legend_Title),
                        legend = c(..(input$Tab5_KM_Treatment_No_Legend_Position)),
                        legend.labs = rownames(summary(..(datafit_NR())$table)),
                        pval.size = 6, risk.table.height = 0.25,
                        ggtheme = theme_bw() +
                            theme(plot.title = element_text(size = 18, hjust = 0.5)) +
                            theme(legend.title = element_text(colour = "black", size = 15, face = "bold")),
        ...
        output$Tab5_Download_KM_Treatment_Yes_PNG <- downloadHandler(
            filename = function() { paste("KM_Survival_Treatment_Yes", '.png', sep='') },
            content = function(file) {
                png(file, width = input$Tab5_KM_Treatment_Yes_Width,
                    height = input$Tab5_KM_Treatment_Yes_Height, units = "in", res = 1200)
                print(KMR1Plot())
                dev.off()
            }
        )

        output$Tab5_Download_KM_Treatment_Yes_SVG <- downloadHandler(
            filename = function() { paste("KM_Survival_Treatment_Yes", '.svg', sep='') },
            content = function(file) {
                svg(file, width = input$Tab5_KM_Treatment_Yes_Width,
        ...
        output$Tab5_Download_KM_Treatment_No_PNG <- downloadHandler(
            filename = function() { paste("KM_Survival_Treatment_No", '.png', sep='') },
            content = function(file) {
                png(file, width = input$Tab5_KM_Treatment_No_Width,
                    height = input$Tab5_KM_Treatment_No_Height, units = "in", res = 1200)
                print(KMR2Plot())
                dev.off()
            }
        )
        ...
        output$Tab5_Download_KM_Treatment_No_SVG <- downloadHandler(
            filename = function() { paste("KM_Survival_Treatment_No", '.svg', sep='') },
            content = function(file) {
                svg(file, width = input$Tab5_KM_Treatment_No_Width,
        ...
        output$Plot1 = metaRender(renderPlot, { ..(KMR1Plot()) })
        output$Plot2 = metaRender(renderPlot, { ..(KMR2Plot()) })
