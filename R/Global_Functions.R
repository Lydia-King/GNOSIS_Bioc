...
Download_Server_1 <- function(id, datalist, plot) {
    moduleServer(id, function(input, output, session) {
        output$Download_PNG_1 <- downloadHandler(filename = function(){paste(plot, ".png", sep="")}, content = function(file){
            png(file, width = input$Plot_Width_1, height = input$Plot_Height_1, units="in", res = 1200)
            print(plot(datalist[[plot]]()))
            dev.off()})

        output$Download_SVG_1 <- downloadHandler(filename = function(){paste(plot, ".svg", sep="")}, content = function(file){
            svg(file, width = input$Plot_Width_1, height = input$Plot_Height_1)
            print(datalist[[plot]]())
            dev.off()})
    })
}
...
Download_Server_2 <- function(id, datalist, plot) {
    moduleServer(id, function(input, output, session) {
        output$Download_PNG_2 <- downloadHandler(filename = function(){paste(plot, ".png", sep="")}, content = function(file){
            png(file, width = input$Plot_Width_2, height = input$Plot_Height_2, units="in", res = 1200)
            print(datalist[[plot]]())
            dev.off()})

        output$Download_SVG_2 <- downloadHandler(filename = function(){paste(plot, ".svg", sep="")}, content = function(file){
            svg(file, width = input$Plot_Width_2, height = input$Plot_Height_2)
            print(datalist[[plot]]())
            dev.off()})
    })
}
